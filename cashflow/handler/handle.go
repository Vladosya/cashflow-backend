...
func (h *Handler) InitRoutes() *gin.Engine { // обработчик роутов, Создание роутов
	...
	api := router.Group("/api-v1")
	{
		...
		api.POST("/ad/replantAd", h.replantAd)                    // Пересаживание пользователей из одного стола в другой
		api.POST("/ad/getInfoAbTables", h.getInfoAbTables)        // Получить данные о том, кто за каким столом сидит и т.д
		api.GET("/ad/getAllAd", h.getAllAd)                       // Получить все мероприятия за промежуток от сегодняшнего дня + 30 дней
		api.PUT("/ad/changeLimitTable", h.changeLimitTable)       // Изменение количества допустимых столов с игроками в мероприятии
	}
