...
### Изменение количества допустимых столов с игроками в мероприятии (PUT)

```sh
Url: http://localhost:8080/api-v1/ad/changeLimitTable
```js
{
  "body": {
    id_ad: int, <- Идентификатор мероприятия из таблицы ad поле id или таблицы game поле id
    new_limitation_tables: int <- новое значение limitation_tables - кол-во разрешенных столов
  }
}
```sh
RETURN: 
  -status: 200 <- typeof int
  -message: "успешное изменение количества допустимых столов" <- typeof string

POSSIBLE MISTAKES:
  -message: "некорректно переданы данные в body";
  -message: "количество столов не может быть меньше 1";
  -message: "ошибка получения из базы данных, {err}";
  -message: "ошибка преобразования полученных данных, {err}";
  -message: "данного мероприятия не существует";
  -message: "значение не было изменено потому что старое и новое значения совпадают";
  -message: "ошибка обновления из базы данных, {err}";
```  